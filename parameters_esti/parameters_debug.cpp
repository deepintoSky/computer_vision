...
int main()
{
	...
	//寻找角点
	bool flag = findChessboardCorners(image1, boardSize, cornerPoints1, CV_CALIB_CB_ADAPTIVE_THRESH);
	drawChessboardCorners(image1, boardSize, cornerPoints1, flag);
	
	imshow("image1", image1);
	
	
	int num_points = cornerPoints1.size();
	MatrixXd U(2*num_points, 1), m(11, 1), K(2*num_points, 11);
	//定义世界坐标,构造矩阵方程
	int i=0, j=0, k=0, index=0;
	for(k = 7; k >= 0; k--){
		for(i = 4; i >= 0; i--){
			U(2 * index) = cornerPoints1.at(index).x;
			U(2 * index + 1) = cornerPoints1.at(index).y;
